...
void copyFile() {
    ...
    if (!inputFile) {
        std::cerr << "Error: File \"" << sourceFile << "\" does not exist or cannot be opened." << std::endl;
        return;
    }

    std::cout << "Enter the filename to create and copy data into: ";
    std::cin >> destinationFile;

    std::ofstream outputFile(destinationFile.c_str());
    if (!outputFile) {
        std::cerr << "Error: Could not create file \"" << destinationFile << "\"." << std::endl;
        return;
    }
    ...
    std::cout << "File \"" << sourceFile << "\" copied successfully to \"" << destinationFile << "\"." << std::endl;
